...
def main():
    ...
    logger.info("[OK] Greenhouse model khoi tao xong")
    ...
    logger.info("[OK] 6 tasks da tao xong")
    ...
    scheduler.register_task(
        "MQTT",      mqtt_task.run,
        interval=config.TASK_INTERVAL_MQTT,        priority=8,
    )
    scheduler.register_task(
        "Bom",       pump_task.run,
    ...
    scheduler.register_task(
        ...
        interval=config.TASK_INTERVAL_ENVIRONMENT,  priority=5,
    )
    scheduler.register_task(
        "AI",        ai_task.run,
    ...
    try:
        ...
        # Cleanup tat ca resources
        scheduler.stop()
        mqtt_task.disconnect()
